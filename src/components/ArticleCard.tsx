...
const ArticleCard = ({ 
  ...
  if (variant === 'dropdown') {
    return (
      <Link
        href={getArticleUrl()}
        className="block px-2 py-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded text-sm"
        onClick={handleClick}
      >
        <div className="font-medium line-clamp-2 text-right dark:text-white">
  ...
  if (variant === 'compact') {
    return (
      <Link
        ...
        onClick={handleClick}
      >
        <div className="flex gap-3 p-3 bg-white dark:bg-slate-800 rounded-lg shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-sm text-right line-clamp-2 mb-1 hover:underline transition-all duration-200 text-gray-900 dark:text-white">
              {article.articleTitle}
            </h3>
            <div className="flex items-center gap-1 justify-end">
              ...
              <IoTimeOutline className="text-gray-600 dark:text-gray-300 text-xs" />
            </div>
          </div>
        </div>
      </Link>
  ...
  return (
    <Link href={getArticleUrl()} onClick={handleClick} className="block">
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow group">
        {showImage && article.imagePath && article.imagePath.trim() !== '' && (
          <>
            ...
            <div className="p-4">
              <p className="text-gray-600 dark:text-gray-300 text-right line-clamp-3 mb-2 text-sm whitespace-normal break-words">
        ...
        {(!showImage || !article.imagePath || article.imagePath.trim() === '') && (
          <div className="p-4">
            <h2 className="text-xl font-semibold mb-2 text-right line-clamp-3 whitespace-normal break-words dark:text-white">
              {article.articleTitle}
            </h2>
            <p className="text-gray-600 dark:text-gray-300 mb-4 text-right line-clamp-4 whitespace-normal break-words">
              {article.articleSummary}
            </p>
            <span className="text-sm text-gray-500 dark:text-gray-400">
              {formattedDate}
